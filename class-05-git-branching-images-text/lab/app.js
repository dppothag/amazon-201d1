...
function sumAndMultiply(a, b, c) { //eslint-disable-line
    var sum=a+b+c;
    var product=a*b*c;
    return ([sum,product, a + ' and '+ b +' and '+ c +' sum to '+ sum +'.',
...
function sumArray(sumArr) { //eslint-disable-line
    ...
    for(var counter=0;counter<sumArr.length;counter++) {
        total = (sum(sumArr[counter], total)[0]);
       
        if(counter<sumArr.length-1)
        value=value + sumArr[counter] + ',';
        else
        value=value + sumArr[counter];
    }
    console.log(total);
    return ([total,value+' was passed in as an array of numbers, and '+total+' is their sum.']);
}
...
function multiplyArray(multArr) { //eslint-disable-line
    ...
    for(var counter=0;counter<multArr.length;counter++) {
        totalProduct=(multiply(multArr[counter],totalProduct)[0]);
        if(counter<multArr.length-1)
        value=value + multArr[counter] + ',';
        else
        value=value + multArr[counter];
    }
    console.log(totalProduct);
    return ([totalProduct,'The numbers '+ value +' have a product of '+ totalProduct +'.']);
}
...
function multiplyAnyArray(dynamicArray) { //eslint-disable-line
    ...
    for(var counter=0;counter<dynamicArray.length;counter++) {
        total=(multiply(dynamicArray[counter],total)[0]);
 
        if(counter<dynamicArray.length-1)
        value=value + dynamicArray[counter] + ',';
        else
        value=value + dynamicArray[counter];
    }
    console.log(total);
    return ([total,'The numbers '+value+' have a product of '+total+'.']);
